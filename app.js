...
async function teamReportsPage(user,team){
  teamModuleShell(team,'REPORTS','Premium PDF, player performance exports, match history and team analytics.',`<section class="ct-report-hero v350"><div><strong>CRICTRACK PREMIUM REPORTS</strong><span>Team performance • Player leaders • Opponents • Match history</span></div><b>V3.5</b></section><div id="reportSummary" class="ct-match-summary"></div><div class="ct-report-actions v350"><button id="printTeamReport">PREMIUM PDF</button><button id="downloadMatchesCsv">MATCH CSV</button><button id="downloadPlayersCsv">PLAYER STATS CSV</button><button id="copyReportSummary">COPY SUMMARY</button></div><div id="reportPreview" class="ct-report-preview v350"><div class="loading-card">Building premium report…</div></div>`);
  const preview=document.getElementById('reportPreview');
  try{
    const snap=await getDocs(collection(db,'teams',team.teamId,'matches'));
    const matches=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>String(b.date||'').localeCompare(String(a.date||'')));
    const done=matches.filter(m=>m.status==='completed');
    const teamName=team.name||team.teamId||'Team';
    let wins=0,losses=0,ties=0,runsFor=0,runsAgainst=0,totalTeamWkts=0,highestTeamScore=0,lowestTeamScore=null;
    const batting={},bowling={},opponents={};
    const recent=[];
    for(const m of done){
      const result=String(m.resultText||'');let outcome='—';
      if(/tied/i.test(result)){ties++;outcome='T';}
      else if(result.toLowerCase().startsWith(String(teamName).toLowerCase()+' won')){wins++;outcome='W';}
    ...
    document.getElementById('reportSummary').innerHTML=`<div><b>${done.length}</b><span>Completed</span></div><div><b>${wins}</b><span>Wins</span></div><div><b>${winRate.toFixed(0)}%</b><span>Win Rate</span></div>`;
    preview.innerHTML=`
      <section class="ct-report-preview-head"><div><span>TEAM PERFORMANCE REPORT</span><h3>${esc(teamName)}</h3><p>Team ID ${esc(team.teamId||'—')} • ${esc(team.city||'City not set')} • ${esc(team.homeGround||'Home ground not set')}</p></div><div class="ct-report-form">${form.length?form.map(x=>`<b class="${x==='W'?'win':x==='L'?'loss':'tie'}">${x}</b>`).join(''):'<small>No recent form</small>'}</div></section>
      <div class="ct-report-kpis"><div><b>${runsFor}</b><span>Runs Scored</span></div><div><b>${runsAgainst}</b><span>Runs Conceded</span></div><div><b>${highestTeamScore}</b><span>Highest Score</span></div><div><b>${totalTeamWkts}</b><span>Wickets</span></div></div>
      <div class="ct-report-two-col">
        <section><h4>BATTING LEADERS</h4>${batLeaders.slice(0,5).map((x,i)=>`<div class="ct-report-leader"><b>${i+1}</b><span><strong>${esc(x.name)}</strong><small>${x.runs} runs • HS ${x.high} • SR ${x.balls?((x.runs/x.balls)*100).toFixed(1):'0.0'} • 4s ${x.fours} • 6s ${x.sixes}</small></span></div>`).join('')||'<p>No batting data yet.</p>'}</section>
        <section><h4>BOWLING LEADERS</h4>${bowlLeaders.slice(0,5).map((x,i)=>`<div class="ct-report-leader"><b>${i+1}</b><span><strong>${esc(x.name)}</strong><small>${x.wickets} wkts • Best ${x.bestW}/${x.bestR===9999?0:x.bestR} • Eco ${x.balls?((x.runs*6)/x.balls).toFixed(2):'0.00'}</small></span></div>`).join('')||'<p>No bowling data yet.</p>'}</section>
      </div>
      <section class="ct-report-section"><h4>OPPONENT SNAPSHOT</h4><div class="ct-report-opps">${oppLeaders.slice(0,4).map(x=>`<div><strong>${esc(x.name)}</strong><span>${x.played} M • ${x.wins} W • ${x.losses} L • ${x.ties} T</span><small>${x.played?((x.wins/x.played)*100).toFixed(0):0}% win rate</small></div>`).join('')||'<p>No opponent data yet.</p>'}</div></section>
      <section class="ct-report-section"><h4>RECENT MATCHES</h4>${rows.length?rows.slice(0,8).map(r=>`<article class="ct-report-row"><div><strong>${esc(teamName)} vs ${esc(r.opponent)}</strong><span>${esc(r.date)} • ${esc(r.venue)} • ${esc(r.overs)} overs</span><small>${esc(r.scores||'Score not available')}</small></div><b>${esc(r.result||String(r.status).toUpperCase())}</b></article>`).join(''):'<div class="empty-state"><strong>No matches to report</strong></div>'}</section>`;

    const playerCsvRows=[...batLeaders.map(x=>({player:x.name,type:'Batting',runs:x.runs,balls:x.balls,high:x.high,sr:x.balls?((x.runs/x.balls)*100).toFixed(1):'0.0',fours:x.fours,sixes:x.sixes,wickets:'',best:'',economy:''})),...bowlLeaders.map(x=>({player:x.name,type:'Bowling',runs:'',balls:x.balls,high:'',sr:'',fours:'',sixes:'',wickets:x.wickets,best:`${x.bestW}/${x.bestR===9999?0:x.bestR}`,economy:x.balls?((x.runs*6)/x.balls).toFixed(2):'0.00'}))];
    const q=v=>'"'+String(v??'').replaceAll('"','""')+'"';
    document.getElementById('downloadMatchesCsv').onclick=()=>{const csv=['Date,Opponent,Venue,Overs,Status,Scores,Result',...rows.map(r=>[r.date,r.opponent,r.venue,r.overs,r.status,r.scores,r.result].map(q).join(','))].join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));a.download=`CricTrack_${String(team.teamId||'Team').replace(/[^a-z0-9_-]/gi,'_')}_Matches.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);};
    document.getElementById('downloadPlayersCsv').onclick=()=>{const csv=['Player,Category,Runs,Balls,High,Strike Rate,4s,6s,Wickets,Best,Economy',...playerCsvRows.map(r=>[r.player,r.type,r.runs,r.balls,r.high,r.sr,r.fours,r.sixes,r.wickets,r.best,r.economy].map(q).join(','))].join('\n');const a=document.createElement('a');a.href=URL.createObjectURL(new Blob([csv],{type:'text/csv;charset=utf-8'}));a.download=`CricTrack_${String(team.teamId||'Team').replace(/[^a-z0-9_-]/gi,'_')}_Player_Stats.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(a.href),1000);};
    const summaryText=`${teamName} — CricTrack Team Report\nCompleted: ${done.length} | Wins: ${wins} | Losses: ${losses} | Ties: ${ties} | Win Rate: ${winRate.toFixed(0)}%\nRuns: ${runsFor} scored / ${runsAgainst} conceded | Highest: ${highestTeamScore}\nTop Batter: ${batLeaders[0]?`${batLeaders[0].name} — ${batLeaders[0].runs} runs`:'—'}\nTop Bowler: ${bowlLeaders[0]?`${bowlLeaders[0].name} — ${bowlLeaders[0].wickets} wickets`:'—'}`;
    document.getElementById('copyReportSummary').onclick=async()=>{try{await navigator.clipboard.writeText(summaryText);alert('Report summary copied.');}catch{alert(summaryText);}};
    document.getElementById('printTeamReport').onclick=()=>{
      const w=window.open('','_blank');if(!w){alert('Allow pop-ups to create the PDF report.');return;}
      const logo=team.logoDataUrl?`<img class="crest" src="${esc(team.logoDataUrl)}" alt="Team logo">`:`<div class="crest fallback">${esc((team.shortName||teamName).slice(0,3).toUpperCase())}</div>`;
      const batRows=batLeaders.length?batLeaders.map((x,i)=>`<tr><td>${i+1}</td><td><b>${esc(x.name)}</b></td><td>${x.runs}</td><td>${x.balls}</td><td>${x.high}</td><td>${x.balls?((x.runs/x.balls)*100).toFixed(1):'0.0'}</td><td>${x.fours}</td><td>${x.sixes}</td></tr>`).join(''):'<tr><td colspan="8">No batting data</td></tr>';
      const bowlRows=bowlLeaders.length?bowlLeaders.map((x,i)=>`<tr><td>${i+1}</td><td><b>${esc(x.name)}</b></td><td>${Math.floor(x.balls/6)}.${x.balls%6}</td><td>${x.runs}</td><td>${x.wickets}</td><td>${x.bestW}/${x.bestR===9999?0:x.bestR}</td><td>${x.balls?((x.runs*6)/x.balls).toFixed(2):'0.00'}</td><td>${x.wides}/${x.noBalls}</td></tr>`).join(''):'<tr><td colspan="8">No bowling data</td></tr>';
      const oppRows=oppLeaders.length?oppLeaders.map(x=>`<tr><td><b>${esc(x.name)}</b></td><td>${x.played}</td><td>${x.wins}</td><td>${x.losses}</td><td>${x.ties}</td><td>${x.runsFor}-${x.runsAgainst}</td><td>${x.played?((x.wins/x.played)*100).toFixed(0):0}%</td></tr>`).join(''):'<tr><td colspan="7">No opponent data</td></tr>';
      const matchCards=rows.length?rows.map((r,i)=>`<div class="match"><div class="num">${i+1}</div><div><h4>${esc(teamName)} vs ${esc(r.opponent)}</h4><p>${esc(r.date||'Date not set')} • ${esc(r.venue||'Venue not set')} • ${esc(r.overs||'—')} overs</p><strong>${esc(r.scores||'Score not available')}</strong></div><span class="result">${esc(r.result||String(r.status).toUpperCase())}</span></div>`).join(''):'<p>No matches stored.</p>';
      const gen=new Date().toLocaleString();
      w.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(teamName)} - CricTrack Premium Report</title><style>
        *{box-sizing:border-box}body{margin:0;font-family:Arial,Helvetica,sans-serif;color:#10213b;background:#edf3fb} @page{size:A4;margin:10mm}
        .page{width:190mm;min-height:277mm;margin:8mm auto;background:#fff;padding:12mm;border-radius:5mm;box-shadow:0 8px 35px #b8c6da;page-break-after:always}.page:last-child{page-break-after:auto}
        .hero{background:linear-gradient(135deg,#061b3a,#0b5fd7 65%,#05357c);color:#fff;border-radius:6mm;padding:10mm;display:flex;justify-content:space-between;align-items:center;gap:8mm;position:relative;overflow:hidden}.hero:after{content:'';position:absolute;width:70mm;height:70mm;border:15mm solid rgba(255,255,255,.04);border-radius:50%;right:-25mm;top:-25mm}.brand{font-size:9pt;letter-spacing:1.2pt;font-weight:800;color:#9ed3ff}.hero h1{font-size:26pt;margin:2mm 0 1mm}.hero p{margin:0;color:#d8eaff;font-size:9pt}.crest{width:26mm;height:26mm;border-radius:6mm;object-fit:cover;border:1.5mm solid rgba(255,255,255,.35);background:#fff}.crest.fallback{display:grid;place-items:center;background:#082b61;color:#fff;font-weight:900;font-size:14pt}
        .section-title{display:flex;justify-content:space-between;align-items:end;margin:9mm 0 4mm;border-bottom:1px solid #d7e2f0;padding-bottom:2mm}.section-title h2{margin:0;color:#0b3f86;font-size:15pt}.section-title span{font-size:8pt;color:#70839c}.kpis{display:grid;grid-template-columns:repeat(4,1fr);gap:3mm;margin:5mm 0}.kpi{border:1px solid #d4e1f0;border-radius:4mm;padding:5mm 3mm;text-align:center;background:linear-gradient(180deg,#f8fbff,#eef5ff)}.kpi b{display:block;font-size:20pt;color:#0b5fd7}.kpi span{font-size:7.5pt;font-weight:800;color:#657895;text-transform:uppercase}.form{display:flex;gap:2mm}.form b{display:grid;place-items:center;width:8mm;height:8mm;border-radius:50%;color:#fff;font-size:8pt}.form .W{background:#11a765}.form .L{background:#e14343}.form .T{background:#d99a00}.insight{display:grid;grid-template-columns:1.1fr .9fr;gap:4mm}.box{border:1px solid #d7e2f0;border-radius:4mm;padding:5mm}.box h3{margin:0 0 3mm;color:#0b3f86;font-size:10pt}.statline{display:flex;justify-content:space-between;padding:2.2mm 0;border-bottom:1px solid #edf1f6;font-size:8.5pt}.statline:last-child{border:0}.leader{display:flex;align-items:center;gap:3mm;padding:2.2mm 0;border-bottom:1px solid #edf1f6}.leader:last-child{border:0}.leader i{font-style:normal;width:7mm;height:7mm;border-radius:50%;display:grid;place-items:center;background:#0b5fd7;color:#fff;font-size:7pt;font-weight:900}.leader b{font-size:8.5pt}.leader small{display:block;color:#71839b;font-size:7pt;margin-top:1mm}
        table{width:100%;border-collapse:collapse;font-size:7.3pt}th{background:#0a3d83;color:#fff;text-align:left;padding:2.5mm 2mm}td{padding:2.4mm 2mm;border-bottom:1px solid #e7edf5}tr:nth-child(even) td{background:#f8fbff}.match{display:grid;grid-template-columns:9mm 1fr 45mm;gap:3mm;align-items:center;padding:3.5mm;border:1px solid #dce5f0;border-radius:3mm;margin-bottom:2.5mm;break-inside:avoid}.num{width:8mm;height:8mm;border-radius:50%;display:grid;place-items:center;background:#0b5fd7;color:#fff;font-weight:900;font-size:7pt}.match h4{margin:0;font-size:9pt}.match p{margin:1mm 0;color:#75869d;font-size:7pt}.match strong{font-size:7.5pt}.result{text-align:right;color:#0b6b3b;font-weight:800;font-size:7.5pt}.footer{margin-top:7mm;border-top:1px solid #dce5f0;padding-top:3mm;display:flex;justify-content:space-between;color:#8391a4;font-size:7pt}.note{margin-top:5mm;background:#edf6ff;border-left:1.5mm solid #0b5fd7;padding:4mm;border-radius:2mm;font-size:8pt;color:#37516f}@media print{body{background:#fff}.page{margin:0;box-shadow:none;border-radius:0;width:auto;min-height:auto;padding:0}.page+.page{padding-top:0}}
      </style></head><body>
      <section class="page"><div class="hero"><div>${logo}</div><div style="flex:1"><div class="brand">CRICTRACK • PREMIUM TEAM PERFORMANCE REPORT</div><h1>${esc(teamName)}</h1><p>Team ID: ${esc(team.teamId||'—')} • ${esc(team.city||'City not set')} • ${esc(team.homeGround||'Home ground not set')}</p></div></div>
      <div class="section-title"><h2>Season Overview</h2><div class="form">${form.map(x=>`<b class="${x}">${x}</b>`).join('')}</div></div><div class="kpis"><div class="kpi"><b>${done.length}</b><span>Matches</span></div><div class="kpi"><b>${wins}</b><span>Wins</span></div><div class="kpi"><b>${losses}</b><span>Losses</span></div><div class="kpi"><b>${winRate.toFixed(0)}%</b><span>Win Rate</span></div><div class="kpi"><b>${runsFor}</b><span>Runs Scored</span></div><div class="kpi"><b>${runsAgainst}</b><span>Conceded</span></div><div class="kpi"><b>${highestTeamScore}</b><span>Highest Score</span></div><div class="kpi"><b>${totalTeamWkts}</b><span>Wickets</span></div></div>
      <div class="insight"><div class="box"><h3>Team Performance</h3><div class="statline"><span>Average team score</span><b>${avgRuns.toFixed(1)}</b></div><div class="statline"><span>Net runs</span><b>${runsFor-runsAgainst}</b></div><div class="statline"><span>Lowest team score</span><b>${lowestTeamScore??0}</b></div><div class="statline"><span>Ties / No result</span><b>${ties}</b></div></div><div class="box"><h3>Top Performers</h3>${batLeaders[0]?`<div class="leader"><i>B</i><div><b>${esc(batLeaders[0].name)}</b><small>${batLeaders[0].runs} runs • HS ${batLeaders[0].high}</small></div></div>`:''}${bowlLeaders[0]?`<div class="leader"><i>W</i><div><b>${esc(bowlLeaders[0].name)}</b><small>${bowlLeaders[0].wickets} wickets • Best ${bowlLeaders[0].bestW}/${bowlLeaders[0].bestR===9999?0:bowlLeaders[0].bestR}</small></div></div>`:''}</div></div><div class="note">This report is generated only from matches and player statistics stored in CricTrack. No sample performance data is inserted.</div><div class="footer"><span>CricTrack • Track Every Run. Celebrate Every Win.</span><span>Generated ${esc(gen)}</span></div></section>
      <section class="page"><div class="section-title"><h2>Batting Performance</h2><span>${batLeaders.length} players</span></div><table><thead><tr><th>#</th><th>Batter</th><th>Runs</th><th>Balls</th><th>HS</th><th>SR</th><th>4s</th><th>6s</th></tr></thead><tbody>${batRows}</tbody></table><div class="section-title"><h2>Bowling Performance</h2><span>${bowlLeaders.length} bowlers</span></div><table><thead><tr><th>#</th><th>Bowler</th><th>Overs</th><th>Runs</th><th>Wkts</th><th>Best</th><th>Eco</th><th>WD/NB</th></tr></thead><tbody>${bowlRows}</tbody></table><div class="section-title"><h2>Head-to-Head</h2><span>${oppLeaders.length} opponents</span></div><table><thead><tr><th>Opponent</th><th>P</th><th>W</th><th>L</th><th>T</th><th>Runs</th><th>Win%</th></tr></thead><tbody>${oppRows}</tbody></table><div class="footer"><span>${esc(teamName)} • Performance Detail</span><span>Page 2</span></div></section>
      <section class="page"><div class="section-title"><h2>Complete Match History</h2><span>${rows.length} stored matches</span></div>${matchCards}<div class="footer"><span>${esc(teamName)} • Match History</span><span>Page 3+</span></div></section>
      <script>window.onload=()=>setTimeout(()=>window.print(),250);<\/script></body></html>`);w.document.close();
    };
