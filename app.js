...
function home(){
  shell(`<main class="ct-home-dark">
    ...
    <section class="ct-registration-grid">
      <button class="ct-registration-card ct-reg-team" data-register="team"><span class="ct-trial-ribbon">1 FREE MATCH</span><div class="ct-reg-icon">👥</div><div><h2>TEAM REGISTRATION</h2><p>Register your team, create your Team ID and play your first full match free.</p><strong>REGISTER TEAM →</strong></div></button>
      <button class="ct-registration-card ct-reg-tournament" data-register="tournament"><div class="ct-reg-icon">🏆</div><div><h2>TOURNAMENT REGISTRATION</h2><p>Register & setup free. Subscription is required only when you start matches.</p><strong>REGISTER TOURNAMENT →</strong></div></button>
    </section>

    <section class="ct-login-grid" aria-label="CricTrack logins">
      <button class="ct-login-tile team" data-login="team"><div>👥</div><strong>TEAM LOGIN</strong><span>Login to your team account</span><i>LOGIN →</i></button>
      <button class="ct-login-tile tournament" data-login="tournament"><div>🏆</div><strong>TOURNAMENT LOGIN</strong><span>Login and continue setup</span><i>LOGIN →</i></button>
      <button class="ct-login-tile scorer" data-login="scorer"><div>🧑‍💻</div><strong>SCORER LOGIN</strong><span>Score assigned matches</span><i>LOGIN →</i></button>
    </section>
...
function teamRegistrationPage(){
  shell(`<main class="login-wrap ct-register-wrap"><button class="back" id="back">← Back to Home</button><form class="login-card ct-self-register" id="teamRegisterForm">
    <div class="login-badge">🏏 &nbsp;TEAM REGISTRATION • 1 FREE MATCH</div><h2 class="login-title">Register Your Cricket Team</h2><p class="login-copy">Create your own Team ID and password. Login works immediately after successful registration.</p>
    ...
    <div class="ct-trial-note"><strong>🎁 Free Trial</strong><span>Your first full match is free. Subscription is required to start additional matches after the trial is used.</span></div>
...
async function registerTeam(e){
  ...
  try{
    ...
    ok.innerHTML=`<strong>✅ Team registered successfully</strong><span>Team ID: <b>${esc(teamId)}</b></span><span>1 free full match is ready.</span><div class="ct-success-actions"><button type="button" id="saveTeamPdf">Save Registration PDF</button><button type="button" id="openTeamNow">Open Team Dashboard</button></div>`;ok.style.display='grid';
    document.getElementById('saveTeamPdf').onclick=()=>printRegistrationSummary('Team Registration',{Name:data.name,'Team ID':teamId,City:data.city,Manager:data.managerName,Mobile:data.mobile,'Trial':'1 Free Match'});
...
function tournamentRegistrationPage(){
  shell(`<main class="login-wrap ct-register-wrap"><button class="back" id="back">← Back to Home</button><form class="login-card ct-self-register" id="tourRegisterForm">
    <div class="login-badge">🏆 &nbsp;TOURNAMENT REGISTRATION</div><h2 class="login-title">Register Your Tournament</h2><p class="login-copy">Create your Tournament ID and login immediately. Setup is free; subscription is required only to start matches.</p>
    ...
    <div class="ct-trial-note tournament"><strong>🏆 Setup Mode</strong><span>Login, add teams, venues, rules and draft fixtures for free. An active subscription is required before any match can start.</span></div>
...
async function registerTournament(e){
  ...
  try{
    ...
    ok.innerHTML=`<strong>✅ Tournament registered successfully</strong><span>Tournament ID: <b>${esc(tournamentId)}</b></span><span>Setup workspace is ready. Match Start remains locked until subscription activation.</span><div class="ct-success-actions"><button type="button" id="saveTourPdf">Save Registration PDF</button><button type="button" id="openTourNow">Open Tournament Dashboard</button></div>`;ok.style.display='grid';
    document.getElementById('saveTourPdf').onclick=()=>printRegistrationSummary('Tournament Registration',{Name:data.name,'Tournament ID':tournamentId,City:data.city,Organizer:data.organizerName,Mobile:data.mobile,'Access':'Setup Free • Match Start Requires Subscription'});
...
function teamDashboard(user,team){const used=Number(team.trialMatchesUsed||0),allowed=Number(team.trialMatchesAllowed??1),trialAvailable=used<allowed;const sub=team.subscriptionStatus||'trial';shell(`<main class="dashboard team-dashboard"><div class="team-top"><div><div class="team-chip">🏏 TEAM ACCOUNT</div><h2>${esc(team.name||team.teamId)}</h2><p>ID: ${esc(team.teamId||'')}</p></div><button class="logout" id="teamLogout">Logout</button></div><div class="team-welcome"><strong>WELCOME BACK</strong><h3>${esc((team.name||'TEAM').toUpperCase())}</h3><span>${trialAvailable?`🎁 Free Match Available • ${used}/${allowed} used`:`Subscription: ${esc(sub)}`}</span></div><div class="team-dashboard-grid"><button>👤<strong>Team Profile</strong></button><button>👥<strong>Players & Squad</strong></button><button id="teamCreateMatch">🗓️<strong>Create Match</strong></button><button>📡<strong>Live Scoring</strong></button><button>📋<strong>Matches</strong></button><button>📊<strong>Statistics</strong></button><button>🤝<strong>Opponents</strong></button><button>📄<strong>Reports</strong></button><button>⚙️<strong>Settings</strong></button></div><div class="phase-note">${trialAvailable?'Your first full match is free. Match engine implementation comes in the next development phases.':'Trial used. An active subscription will be required before another match can start.'}</div></main>`,'TEAM DASHBOARD • SECURE');document.getElementById('teamLogout').onclick=async()=>{await signOut(auth);home();};document.getElementById('teamCreateMatch').onclick=()=>alert(trialAvailable?'Free-match entitlement confirmed. Create Match module is the next coding phase.':'Trial match already used. Subscription will be required to start another match.');}
function tournamentDashboard(user,t){shell(`<main class="dashboard ct-tournament-dashboard"><div class="team-top"><div><div class="team-chip">🏆 TOURNAMENT WORKSPACE</div><h2>${esc(t.name||t.tournamentId)}</h2><p>ID: ${esc(t.tournamentId||'')}</p></div><button class="logout" id="tourLogout">Logout</button></div><div class="ct-subscription-banner ${t.canStartMatch?'active':''}"><strong>${t.canStartMatch?'✅ Subscription Active':'🔒 Setup Mode • Match Start Locked'}</strong><span>${t.canStartMatch?'Matches can be started.':'You can complete tournament setup now. Activate subscription before starting any match.'}</span></div><div class="admin-grid ct-tour-grid"><button class="admin-action"><div>👥</div><strong>Teams</strong><small>Add / invite tournament teams.</small></button><button class="admin-action"><div>🧍</div><strong>Players</strong><small>Registration and approvals.</small></button><button class="admin-action"><div>📅</div><strong>Fixtures</strong><small>Create draft fixtures.</small></button><button class="admin-action" id="tourStartMatch"><div>📡</div><strong>Start Match</strong><small>${t.canStartMatch?'Ready to start':'Subscription required'}</small></button><button class="admin-action"><div>🏟️</div><strong>Venues</strong><small>Grounds and availability.</small></button><button class="admin-action"><div>⚙️</div><strong>Settings</strong><small>Rules, branding and security.</small></button></div></main>`,'TOURNAMENT • SETUP WORKSPACE');document.getElementById('tourLogout').onclick=async()=>{await signOut(auth);home();};document.getElementById('tourStartMatch').onclick=()=>alert(t.canStartMatch?'Start Match flow will be enabled in the Tournament Match phase.':'Subscription required to start tournament matches. Your setup data remains safe.');}
