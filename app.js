...
function teamDashboard(user,team){
  ...
  ['teamMatches','teamStatistics','teamOpponents','teamReports','teamSettings'].forEach(id=>{document.getElementById(id).onclick=()=>alert(document.getElementById(id).querySelector('strong').textContent+' module is being connected in the next phase.');});
}
...
async function teamSquadPage(user,team){
  teamModuleShell(team,'PLAYERS & SQUAD','Manage players, roles and your Playing XI',`
    ...
    <div id="quickAddWrap" class="ct-player-form-wrap" hidden>
      <form id="quickAddForm" class="ct-premium-form compact">
        <h3 class="ct-quick-title">QUICK ADD SQUAD</h3><p class="ct-quick-copy">One player per line: <b>Name, Jersey No, Role</b>. Example: Ravi, 7, Batter</p>
        <textarea id="quickAddText" class="ct-quick-text" rows="11" placeholder="Ravi, 7, Batter\nKiran, 18, Bowler\nArjun, 10, All-rounder"></textarea>
  ...
  document.getElementById('quickAddForm').onsubmit=async e=>{e.preventDefault();const btn=document.getElementById('saveQuickAdd'),err=document.getElementById('quickAddError');err.style.display='none';try{const lines=document.getElementById('quickAddText').value.split(/\n+/).map(x=>x.trim()).filter(Boolean);if(!lines.length)throw new Error('Enter at least one player.');if(lines.length>25)throw new Error('Add maximum 25 players at a time.');const existingJerseys=new Set(players.map(p=>String(p.jerseyNo||'').trim()).filter(Boolean));const parsed=lines.map((line,i)=>{const a=line.split(',').map(x=>x.trim());const name=a[0]||'';const jersey=a[1]||'';const role=a[2]||'Player';if(!name)throw new Error(`Player name missing on line ${i+1}.`);if(jersey&&existingJerseys.has(jersey))throw new Error(`Jersey ${jersey} already exists.`);if(jersey)existingJerseys.add(jersey);return{name,jerseyNo:jersey,role,battingStyle:'Right Hand',bowlingStyle:'None',isCaptain:false,isViceCaptain:false,isWicketKeeper:false,active:true};});btn.disabled=true;btn.textContent='ADDING…';const selectXI=document.getElementById('quickSelectXI').checked;let xiCount=players.filter(p=>p.playingXI).length;const batch=writeBatch(db);const added=[];for(const d of parsed){const ref=doc(collection(db,'teams',team.teamId,'players'));const playingXI=selectXI&&xiCount<11;if(playingXI)xiCount++;const data={...d,playingXI,createdAt:serverTimestamp(),updatedAt:serverTimestamp()};batch.set(ref,data);added.push({id:ref.id,...data});}await batch.commit();players.push(...added);players.sort((a,b)=>(Number(a.jerseyNo)||999)-(Number(b.jerseyNo)||999)||String(a.name).localeCompare(String(b.name)));document.getElementById('quickAddForm').reset();document.getElementById('quickSelectXI').checked=true;document.getElementById('quickAddWrap').hidden=true;render();}catch(ex){err.textContent=friendlyError(ex);err.style.display='block';}finally{btn.disabled=false;btn.textContent='ADD SQUAD';}};
...
async function teamLiveScoringPage(user,team,preferredMatchId=''){
  let matches=[];try{const snap=await getDocs(collection(db,'teams',team.teamId,'matches'));matches=snap.docs.map(d=>({id:d.id,...d.data()})).filter(m=>m.status==='live');}catch{}
  let match=matches.find(m=>m.id===preferredMatchId)||matches[0];
  ...
  const score=match.score||{runs:0,wickets:0,legalBalls:0,balls:[]};
  const renderScore=()=>{const over=`${Math.floor(score.legalBalls/6)}.${score.legalBalls%6}`;document.getElementById('liveRuns').textContent=score.runs;document.getElementById('liveWkts').textContent=score.wickets;document.getElementById('liveOvers').textContent=over;document.getElementById('liveRecent').innerHTML=(score.balls||[]).slice(-12).map(x=>`<span>${esc(x.label)}</span>`).join('')||'<small>No balls recorded yet</small>';};
  teamModuleShell(team,'LIVE SCORING',`${match.teamAName||team.name||team.teamId} vs ${match.teamBName||'Opponent'} • ${match.overs} overs`,`
    <section class="ct-live-score-card"><div class="ct-live-match-title"><span>LIVE</span><strong>${esc(match.teamAName||team.name||team.teamId)} vs ${esc(match.teamBName||'Opponent')}</strong><small>${esc(match.venue||'')} • ${esc(match.matchType||'')}</small></div>
    ...
    <div class="ct-score-buttons">${[0,1,2,3,4,6].map(n=>`<button data-run="${n}">${n}</button>`).join('')}<button class="wicket" id="scoreWicket">WICKET</button><button class="undo" id="scoreUndo">UNDO</button></div>
    <div class="ct-recent-balls" id="liveRecent"></div><div class="error" id="liveError"></div></section>`);
  const save=async()=>{await updateDoc(doc(db,'teams',team.teamId,'matches',match.id),{score,updatedAt:serverTimestamp()});renderScore();};
  document.querySelectorAll('[data-run]').forEach(b=>b.onclick=async()=>{const n=Number(b.dataset.run);score.runs+=n;score.legalBalls++;score.balls.push({label:String(n),runs:n,wicket:false});try{await save();}catch(ex){document.getElementById('liveError').textContent=friendlyError(ex);document.getElementById('liveError').style.display='block';}});
  document.getElementById('scoreWicket').onclick=async()=>{if(score.wickets>=10)return;score.wickets++;score.legalBalls++;score.balls.push({label:'W',runs:0,wicket:true});try{await save();}catch(ex){alert(friendlyError(ex));}};
  document.getElementById('scoreUndo').onclick=async()=>{const last=score.balls.pop();if(!last)return;score.runs=Math.max(0,score.runs-Number(last.runs||0));if(last.wicket)score.wickets=Math.max(0,score.wickets-1);score.legalBalls=Math.max(0,score.legalBalls-1);try{await save();}catch(ex){alert(friendlyError(ex));}};renderScore();
}
