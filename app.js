...
function teamDashboard(user,team){const used=Number(team.trialMatchesUsed||0),allowed=Number(team.trialMatchesAllowed??1),trialAvailable=used<allowed;const sub=team.subscriptionStatus||'trial';shell(`<main class="dashboard ct-dashboard-shell ct-team-dashboard-v240"><div class="ct-dash-hero team"><div><div class="ct-dash-role">👥 TEAM WORKSPACE</div><h2>${esc(team.name||team.teamId)}</h2><p>Team ID: ${esc(team.teamId||'')} • Secure access</p></div><button class="ct-dash-logout" id="teamLogout">Logout</button></div><div class="ct-dash-stats team-stats"><div><b>✓</b><span>Account Active</span></div><div><b>☁</b><span>Cloud Sync</span></div><div><b>🏏</b><span>Match Ready</span></div><div><b>🔒</b><span>Secure</span></div></div><div class="ct-dash-title"><strong>TEAM DASHBOARD</strong><span>Manage • Score • Analyze</span></div><div class="ct-dash-grid team-grid-v240"><button class="ct-dash-card green"><div>🛡️</div><strong>TEAM PROFILE</strong><small>Identity, logo & team details</small></button><button class="ct-dash-card green"><div>👥</div><strong>PLAYERS & SQUAD</strong><small>Players, roles & Playing XI</small></button><button class="ct-dash-card blue" id="teamCreateMatch"><div>➕</div><strong>CREATE MATCH</strong><small>Opponent, venue & match rules</small></button><button class="ct-dash-card blue"><div>📡</div><strong>LIVE SCORING</strong><small>Fast ball-by-ball scoring</small></button><button class="ct-dash-card purple"><div>📋</div><strong>MATCHES</strong><small>Upcoming, live & completed</small></button><button class="ct-dash-card purple"><div>📊</div><strong>STATISTICS</strong><small>Team & player performance</small></button><button class="ct-dash-card orange"><div>🤝</div><strong>OPPONENTS</strong><small>Head-to-head & history</small></button><button class="ct-dash-card orange"><div>📄</div><strong>REPORTS</strong><small>PDF, Excel & share reports</small></button><button class="ct-dash-card red wide"><div>⚙️</div><strong>TEAM SETTINGS</strong><small>Profile, security & preferences</small></button></div><div class="ct-dash-secure">🛡️ <span><strong>Team workspace securely connected</strong><small>Real-time • Cloud synchronized</small></span></div></main>`,'TEAM DASHBOARD • SECURE');document.getElementById('teamLogout').onclick=async()=>{await signOut(auth);home();};document.getElementById('teamCreateMatch').onclick=()=>alert(trialAvailable?'Match access available. Create Match module is the next coding phase.':'Subscription is required to start another match.');}
